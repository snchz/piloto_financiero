...
Para actualizar el servidor con los últimos cambios publicados en GitHub Container Registry, utiliza los siguientes comandos en la ruta donde tengas tu `compose.yaml`:
...
- `docker compose pull`: Descarga la última versión de la imagen preconstruida `ghcr.io/snchz/piloto_financiero:latest` desde GitHub.
- `docker compose up -d`: Recrea el contenedor aplicando la nueva imagen de forma transparente y sin interrupciones.
